...
public class Viewport
{
    ...
    {
        viewportWidth = initWidth;
        viewportHeight = initHeight;
    }
    ...
    {
        if ((initLevelWidth >= viewportWidth) && (initLevelHeight >= viewportHeight))
        {
            gameWorldWidth = initLevelWidth;
            gameWorldHeight = initLevelHeight;
        }
    }
    ...
    {
        // MOVE
        if(!(viewportX + incX + gameWorldWidth < screenWidth || viewportX + incX > 0))
            viewportX += incX;
        if(!(viewportY + incY + viewportHeight+100 < gameWorldHeight || viewportY + incY > 100))
            viewportY += incY;
    }
    ...
    public void move(int incX, int incY)
    {
        ...
        if (viewportX > (gameWorldWidth - viewportWidth))
            viewportX = gameWorldWidth - viewportWidth;
        if (viewportY > (gameWorldHeight - viewportHeight))
            viewportY = gameWorldHeight - viewportHeight;
    }
}