 ...
 * @author Brian Sabz
 */
public class CombinatorialSearch2 {
    ...
    public static void main(String[] args) {
        ...
        Graph g = new Graph(false, numVertices);
        //AdjMatrixGraph adj = new AdjMatrixGraph(numVertices, false);
        
        // INITIALIZE GRAPH
        for (int i = 2; i < graphData.length; i++) {
            g.insertEdge(graphData[i][0], graphData[i][1], false);
        ...
        int a[] = new int[g.getNumVertices()];

        backtrack(g, a, 0, g.getNumVertices());
    ...
    public static void backtrack(Graph g, int a[], int k, int n) {
     ...
     * @param a
     */
    public static void processSolution(Graph g, int a[]) {
        int currentEstimate = 0;
        for (int i = 0; i < a.length - 1; i++) {
            for (int j = i + 1; j < a.length; j++) {
                if (g.edgeExists(a[i] + 1, a[j] + 1)) {
